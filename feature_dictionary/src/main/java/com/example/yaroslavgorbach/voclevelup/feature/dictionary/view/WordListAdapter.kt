...
internal class WordListAdapter(
    ...
    inner class WordVh(private val bind: ItemWordBinding) : RecyclerView.ViewHolder(bind.root) {

        init {
            itemView.setOnClickListener {
                onWordClick(getItem(adapterPosition), it)
            }
        }
        ...
        fun bind(word: Word) = with(bind) {
            ...
            itemView.transitionName = "word_${getItem(adapterPosition).text}"
        }
    }
}